...
# ======================================SC restricted weights (non-negative)  ======================================
...
# ==================== SBC restricted weights (non-negative) ======================================
...
# Define colors: UK (24th column) in blue, others in grey
df_long <- df_long %>%
  mutate(Color = ifelse(Country == "HK", "HK", "Donor Countries"))
...
# ============ SC/SBC with signed weights sum to 1 ======================================
predata <- donor_df[1:T0,]
X1 <- as.matrix(predata$HK)
X0 <- as.matrix(subset(predata[,2:ncol(predata)], select = -HK))
...
donor_trend_pre <- donor_detrend_df[(h+p):T0,] - donor_detrended_pre
synth.trendSC <- synth(data.prep.obj = NULL,
                       X1 = as.matrix(HK_trend_pre), X0 = as.matrix(donor_trend_pre),
                       Z0 = as.matrix(donor_trend_pre), Z1 = as.matrix(HK_trend_pre),
                       custom.v = rep(1,length(HK_trend_pre)),
                       optimxmethod = c("Nelder-Mead", "BFGS"), 
                       genoud = FALSE, quadopt = "ipop", 
                       Margin.ipop = 5e-04, 
                       Sigf.ipop = 5, 
...
weights.trendSC <- synth.trendSC$solution.w

countries <- c("Australia", "Austria", "Korea", "Canada", "Denmark", "France","Germany","Italy", "Netherlands","New Zealand","US")
countries_null <- c("","","","","","","","","","","")



bp <- barplot(
  rbind(c(weights.trendSC),c(weights.SBC), c(weights.sc)),
  beside = TRUE,
  col = c("white","black","grey"),
  names.arg = countries_null,
  legend.text = c("SC trend","SBC", "SC"),
  # xlab = "Weight Index",
  ylab = "weights",
  # main = "Weights Comparison",
  las = 2,         
  args.legend = list(cex = 0.7, box.lty = 0),  
  ylim = c(0,0.6)
)


group_mids <- colMeans(matrix(bp, nrow = 3))  


text(x = group_mids, y = -0.02, labels = countries, xpd = TRUE, srt = 45, adj = 1, cex = 1)


# ============== barplot signed weights ======================

X1 <- as.matrix(HK_trend_pre)
X0 <- as.matrix(donor_trend_pre)

D <- t(X0) %*% X0  # X'X
d <- t(X0) %*% X1  # X'y

A <- matrix(1, nrow = 1, ncol = ncol(X0))  # 约束矩阵
b <- 1  # 约束值

sol <- solve.QP(D, d, t(A), b, meq = 1)  # meq=1 表示等式约束
beta_trend_w1 <- sol$solution



barplot(
  rbind(c(beta_trend_w1),c(beta_SBC_w1), c(beta_SC_w1)),
  beside = TRUE,
  col = c("white","black","grey"),
  names.arg = countries_null,
  legend.text = c("SC trend","SBC", "SC"),
...
text(x = group_mids, y = -1.02, labels = countries, xpd = TRUE, srt = 45, adj = 1, cex = 1)
