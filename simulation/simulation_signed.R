# Simulation code for Model 1-3, weights coefficients sum up to one (can be negative)
...
run_sc_signed <- function(y_matrix, T0) {
  X1 <- as.matrix(y_matrix[1:T0,1])
  X0 <- as.matrix(y_matrix[1:T0,-1])
  ...
  weights.sc <- sol$solution
  
  Y1hat.SC <- as.matrix(y_matrix[1:(T0+Fh),-1]) %*% weights.sc
...
run_sbc_signed <- function(y_matrix, T0) {
...
for (iT0 in 1:3) {
  ...
  for (iphi in 1:3){
    for (j in 1:nsim){
      ...
      MSE_sc <- run_sc_signed(y_matrix,T0 = T0)
      MSE_sbc <- run_sbc_signed(y_matrix,T0 = T0)
