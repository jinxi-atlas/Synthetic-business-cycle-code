# Simulation code for Model 1-3, weights coefficients are non-negative and sum up to one
...
run_sc <- function(y_matrix, T0) {
  ...
  synth.sc <- synth(data.prep.obj = NULL,
                    ...
                    Margin.ipop = 0.0005, Sigf.ipop = 7, 
...
for (iT0 in 1:3) {
  ...
  for (iphi in 1:3){
    for (j in 1:nsim){
      ...
      MSE_sc <- run_sc(y_matrix,T0 = T0)
      MSE_sbc <- run_sbc(y_matrix,T0 = T0)
